...
def r2_upload_sensor(context):
    ...
    yield RunRequest(run_key=f"r2_upload_{context.cursor}")
...
def distributed_r2_sync(context):
    ...
    yield RunRequest(run_key=f"r2_download_{context.cursor}")
...
def distributed_serve(context):
    ...
    yield RunRequest(run_key=f"load_{context.cursor}")
