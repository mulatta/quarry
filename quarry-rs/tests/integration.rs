...
use quarry_rs::build::config::BuildConfig;
...
fn test_pubmed_build() {
    ...
    let stats = quarry_rs::build::pubmed::build_pubmed(
        &config,
        &pubmed_fixtures,
        None,
        &conninfo,
...
fn test_pubmed_progress_skip() {
    ...
    // First build
    let stats1 = quarry_rs::build::pubmed::build_pubmed(
        &config,
        &pubmed_fixtures,
        None,
        &conninfo,
    ...
    let stats2 = quarry_rs::build::pubmed::build_pubmed(
        &config,
        &pubmed_fixtures,
        None,
        &conninfo,
...
fn test_oa_local_build() {
    ...
    let stats = quarry_rs::build::oa::build_oa_local(&config, &oa_fixtures, &conninfo).unwrap();
