"""CSR mmap graph: load and query citation graph built by etl/icite.py.

id_map uses PMIDs (integer, stored as text lines in id_map.bin).
"""
...
class CSRGraph:
    ...
    def __init__(self, csr_dir: Path):
        ...
        # Load id_map: index → pmid (as string), and reverse lookup
        raw_lines = (csr_dir / "id_map.bin").read_text().strip().split("\n")
        self._idx_to_id: list[str] = raw_lines
        self._id_to_idx: dict[str, int] = {pid: i for i, pid in enumerate(raw_lines)}
    ...
    def _resolve(self, node_id: str | int) -> int | None:
        """Resolve a PMID (str or int) to node index."""
    ...
    def neighbors(self, node_id: str | int, direction: str = "forward") -> list[str]:
    ...
    def degree(self, node_id: str | int) -> tuple[int, int]:
    ...
    def shortest_path(
        self, src: str | int, dst: str | int, max_depth: int = 10
    ) -> list[str] | None:
        """Bidirectional BFS shortest path between two nodes.

        Returns list of PMIDs from src to dst, or None if no path.
        """
